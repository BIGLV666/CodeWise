 ...
 * 统一 Feign 请求头拦截器自动装配。
 *
 * <p>身份三件套（X-User-Id/X-User-Name/X-Real-IP）从 {@link UserContext} 透传，
 * 内部 Token 读 {@code codewise.internal-token} 配置（环境变量
 * {@code CODEWISE_INTERNAL_TOKEN}，无默认值——缺失时启动失败即暴露配置遗漏，
 * 与网关、{@code UserAuthInterceptor} 三处必须同值）。</p>
 *
...
public class FeignHeaderAutoConfiguration {
     ...
     * 注册全局 Feign 请求头拦截器。
     *
     * @param internalToken 内部通信 Token，来自配置 codewise.internal-token（无默认值）
     * @return 拦截器实例
     */
    @Bean
