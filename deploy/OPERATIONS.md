...
| RabbitMQ | rabbitmq:3.13 | `rabbitmq-data` | 管理台 15672 未对外暴露，需要时临时映射 |
...
| codewise_question | `question.sql` → `function_question.sql` → `function_question_seed.sql` → `function_test_case_unique_hash.sql` → `event_outbox.sql` |
| codewise_review | `sql/sql.sql` → `migration/V2__review_schedule_index.sql` → `event_outbox.sql` |
...
已有历史数据升级时**不要重建卷**：按 `docs/maintenance/messaging-reliability.md` 手工执行增量 SQL，并排空旧 `judge.queue`/`ai.queue`（管理台 moveTo 后删除）。
...
| 现象 | 排查 |
| --- | --- |
| 服务间 Feign 503/超时 | Nacos 控制台（`localhost:8848/nacos`）确认各服务注册的是容器网段 IP 而非 127.0.0.1 |
