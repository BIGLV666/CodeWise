...
const defaultLookup = (hostname) => nodeLookup(hostname, { all: true });
...
function hexGroupPairToIpv4(pair) {
...
function extractEmbeddedIpv4(ip) {
    ...
    if (hexMapped)
        return hexGroupPairToIpv4(`${hexMapped[1]}:${hexMapped[2]}`);
    const nat64 = lower.match(/^64:ff9b::([0-9a-f]{1,4}):([0-9a-f]{1,4})$/);
    if (nat64)
        return hexGroupPairToIpv4(`${nat64[1]}:${nat64[2]}`);
    const sixToFour = lower.match(/^2002:([0-9a-f]{1,4}):([0-9a-f]{1,4})::/);
    if (sixToFour)
        return hexGroupPairToIpv4(`${sixToFour[1]}:${sixToFour[2]}`);
    return null;
}
...
export function isBlockedIpv6(ip) {
    ...
    if (lower.startsWith('2001:db8:'))
        return true; // 2001:db8::/32 文档
    if (lower.startsWith('2001:0') || lower.startsWith('2001::'))
        return true; // 2001::/23 保留（含 2001::/32 Teredo）
    return false;
}
...
export function parseFetchableUrl(raw, maxUrlLength = 2048) {
 ...
 */
export async function assertPublicHost(hostname, lookupImpl = defaultLookup) {
