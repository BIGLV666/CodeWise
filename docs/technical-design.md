...
同一会话的摘要任务使用 Redisson 锁串行化，数据库更新再通过 MyBatis-Plus `version` 乐观锁保护。原始消息不删除，摘要只是 Prompt 缓存，不作为事实的唯一来源。当前不增加 Redis 摘要缓存，避免小规模阶段引入缓存一致性成本。

流式回答已经输出内容后不再切换模型，避免两个模型的文本拼接到同一条回答；只有首个模型尚未输出任何 chunk 时才允许降级。
