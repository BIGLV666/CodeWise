...
- **Two independent AI paths**: `service-ai` consumes judge-failure events to generate suggestions + in-question SSE follow-ups; `codewise-agent` is a separate FastAPI facade driving a dsh Node tool runtime (cordis plugin tools) that calls back through the Gateway using the user's original Bearer token (self-validates JWT with the shared `JWT_SECRET`; DB tables `agent_*` in `codewise_ai`). Both reach models through an OpenAI-compatible gateway (platform providers from Nacos + per-user custom configs). In Docker it is exposed via nginx `/agentapi/` (frontend builds with `VITE_AGENT_BASE_URL=/agentapi`).
...
- **Long ID 序列化（JS 精度）**：实体 ID 默认 `BIGINT AUTO_INCREMENT`（自增，安全 < 2^53）。只有雪花 ID（`IdType.ASSIGN_ID`，当前仅 `take_down_post.id`）会超过 JS Number 2^53，其 VO 字段必须加 `@JsonSerialize(using = ToStringSerializer.class)` 序列化为字符串（见 `TakeDownPostVo.takeDownId`）。计数类 Long（`likeCount`/`total`/`completed` 等前端要做算术的）保持数字，**不要做全局 Long→String**；未来把自增 ID 改成雪花时，逐个给对应 VO 字段加该注解。
