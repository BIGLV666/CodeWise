...
- 消息通知：统一承载邮件、RabbitMQ 消费和 WebSocket 实时通知。
...
| `service-message` | 8083 | 邮件、消息消费、WebSocket 通知 |
...
例如：`codewise_user`、`codewise_question`、`codewise_review`、`codewise_community`。
...
项目处于持续开发阶段，核心微服务和主要业务链路已经形成。当前重点包括完善网关路由、补充自动化测试、增强判题安全边界，以及继续收敛跨服务异常处理和配置管理。
